...
namespace code {
  ...
  void aarch64Asm::init() {
    ...
    for (int i = 0; i < sections_size; i++)
    {
      ...
      for (int j = 0; j < functions_size; j++){
        ...
        functions[function_name] = &root["sections"][i]["functions"][j]["instructions"];
      }
    }
    ...
    for (Json::Value::Members::iterator itM = members.begin(); itM != members.end(); itM++){
      ...
      for (int i = 0; i < func_cnt; i++) {
        ...
        cout << func_name << endl;
        #endif
        functions[func_name] = &((*funcs)[i]["blocks"]);
      }
    }
    #endif
  }
  ...
  void aarch64Asm::getOpcodeSequence(std::string &function, std::vector<std::string> &s) {
    Json::Value insts = *functions[function];
  ...
  void aarch64Asm::extractRawCode (std::string &function, std::map<INSADDR, std::string> *code, bool replace) {
    #ifdef TRAIN
    Json::Value insts = *functions[function];
    int insts_size = insts.size();
    for (int i = 0; i < insts_size; i++){
    ...
    #else
    Json::Value *blocks = &(*functions[function]);
    int blk_cnt = blocks->size();
    for (int i = 0; i < blk_cnt; i++) {
      Json::Value insts = (*blocks)[i]["instructions"];
      int insts_size = insts.size();
      for (int i = 0; i < insts_size; i++){
        string inst = insts[i]["instruction"].asString();
        string comment = insts[i]["comment"].asString();
  ...
  void aarch64Asm::printRawCode(string &function) {
    map<INSADDR, string> code;
    extractRawCode(function, &code);
    for (map<INSADDR, string>::iterator it = code.begin(); it != code.end(); ++it)
    {
      cout << it->second << endl;
    }
  }

  void aarch64Asm::getFormattedCode(string &function, map<INSADDR, INSTRUCTION> *code) {
    map<INSADDR, string> raw_code;
    extractRawCode(function, &raw_code);
    for (map<INSADDR, string>::iterator it = raw_code.begin(); it != raw_code.end(); ++it)
    {
      INSTRUCTION inst;
  ...
  void aarch64Asm::PrettyPrintCode (map<INSADDR, INSTRUCTION> &code) {
  ...
  void aarch64Asm::memoryLocalize(map<INSADDR, INSTRUCTION> &code) {
    map<ADDR, int> tmp;
    for (map<INSADDR, INSTRUCTION>::iterator it = code.begin(); it != code.end(); ++it){
  ...
  void aarch64Asm::registerLocalize(string &function, map<INSADDR, INSTRUCTION> &code) {
   ...
   */
  void aarch64Asm::localVariableMap(map<INSADDR, INSTRUCTION> &code, map<OPERAND, OPERAND> &localVar) {
    for (map<INSADDR, INSTRUCTION>::iterator it = code.begin(); it != code.end(); ++it) {
  ...
  void aarch64Asm::removeDataTransfer(string &function, map<INSADDR, INSTRUCTION> &code) {
  ...
  void aarch64Asm::buildParam(map<INSADDR, INSTRUCTION> *code) {
    map<AArch64Reg, OPERAND> stack;
    map<INSADDR, INSTRUCTION>::iterator tmp = code->end();
    for (map<INSADDR, INSTRUCTION>::iterator it = code->begin(); it != code->end(); ++it)
    {
      int reg_cnt = 0;
      #ifdef DEBUG
  ...
  void aarch64Asm::fixReturnValue(map<INSADDR, INSTRUCTION> *code) {
    for (map<INSADDR, INSTRUCTION>::iterator it = code->begin(); it != code->end(); ++it)
    {
      if (isCall(it->second.mnemonic))
      {
  ...
  void aarch64Asm::destinationAnalyze(map<INSADDR, INSTRUCTION> &code, map<OPERAND, deque<INSADDR>> &destTable) {
    for (map<INSADDR, INSTRUCTION>::iterator it = code.begin(); it != code.end();++it){
      if(!it->second.visiable){
        continue;
      }
  ...
  void aarch64Asm::buildDDG(map<INSADDR, INSTRUCTION> &code, GRAPH &ddg) {
    map<OPERAND, deque<INSADDR>> destTable;
    destinationAnalyze(code, destTable);
    ...
    for (map<INSADDR, INSTRUCTION>::iterator it = code.begin(); it != code.end(); ++it)
    {
      ...
      ddg.g.push_back(inst);
      index[it->first] = cnt;
      cnt++;
    }
    for (vector<NODE>::iterator it = ddg.g.begin(); it != ddg.g.end(); ++it){
      INSTRUCTION inst = code[it->address];
      #ifdef DEBUG
    ...
    for (vector<NODE>::iterator it = ddg.g.begin(); it != ddg.g.end(); ++it)
    {
      for (vector<INDEX>::iterator ot = it->pre.begin(); ot != it->pre.end(); ++ot){
        if(*ot == -1)
          continue;
        INSADDR destAddr = ddg.g[*ot].address;
        INDEX idx = index[destAddr];
        ddg.g[idx].post.push_back(cnt);
      }
      cnt++;
    }
  }
  ...
  void aarch64Asm::findAllExits (GRAPH &ddg){
    ...
    for (vector<NODE>::iterator it = ddg.g.begin(); it != ddg.g.end(); ++it){
      if (it->post.size() == 0) {
        ddg.roots.push_back(cnt);
      }
      cnt++;
    }
  }

  void aarch64Asm::DDAnalyze(string &function, map<INSADDR, INSTRUCTION> &code, GRAPH &ddg){
    getFormattedCode(function, &code);
    registerLocalize(function, code);
#ifdef DEBUG
    ...
    cout << endl;
#endif
    removeDataTransfer(function, code);
#ifdef DEBUG
  ...
  void aarch64Asm::PrettyPrintGraph(vector<NODE> &g, map<INSADDR, INSTRUCTION> &code) {
  ...
  void aarch64Asm::PrettyPrintExits(GRAPH &ddg){
    for(auto it: ddg.roots){
      cout << ddg.g[it].address << ", ";
    }
  }

  void aarch64Asm::PrettyPrintDDG(map<INSADDR, INSTRUCTION> &code, GRAPH &ddg) {
    cout << "Instructions: " << endl;
    PrettyPrintCode(code);
    cout << endl;
    cout << "Edges: " << endl;
    PrettyPrintGraph(ddg.g, code);
    cout << endl;
    cout << "Exits: " << endl;
    PrettyPrintExits(ddg);
    cout << endl;
  }

  void aarch64Asm::getStoreTable(map<OPERAND, pair<OPERAND, INSADDR>> &storeTable, map<INSADDR, INSTRUCTION> &code) {
    for (map<INSADDR, INSTRUCTION>::iterator it = code.begin(); it != code.end(); it++){
  ...
  void aarch64Asm::removeStore(map<INSADDR, INSTRUCTION> &code) {
    map<OPERAND, pair<OPERAND, INSADDR>> storeTable;
    getStoreTable(storeTable, code);
    for (map<INSADDR, INSTRUCTION>::reverse_iterator it = code.rbegin(); it != code.rend(); ++it)
    {
  ...
  void aarch64Asm::colorLeftNodes (GRAPH &ddg, vector<INDEX> &nodes) {
  ...
  void aarch64Asm::getColorField(GRAPH &ddg) {
    int cnt = 0;
    for (auto n : ddg.g)
    {
  ...
  void aarch64Asm::PrettyPrintColor(GRAPH &ddg) {
    for (COLOR c = 0; c < ddg.all_colors;c++) {
      ...
      {
        cout << ddg.g[n].address << " ";
      }
      cout << endl;
    }
  }

  void aarch64Asm::removeFunctionPreface (map<INSADDR, INSTRUCTION> &code) {
    int cnt = code.size();
    for (map<INSADDR, INSTRUCTION>::reverse_iterator it = code.rbegin(); it != code.rend(); ++it)
    {
  ...
  void aarch64Asm::removeFunctionEpilogue (map<INSADDR, INSTRUCTION> &code) {
    int cnt = 0;
    for (map<INSADDR, INSTRUCTION>::reverse_iterator it = code.rbegin(); it != code.rend(); ++it)
    {
  ...
  void aarch64Asm::removeADRP (map<INSADDR, INSTRUCTION> &code) {
    for (map<INSADDR, INSTRUCTION>::iterator it = code.begin(); it != code.end();++it){
