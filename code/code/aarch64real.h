...
typedef map<INSADDR, std::string> RAWBLOCK;

typedef map<BLOCK_ADDR, RAWBLOCK> RAWFUNC;
...
typedef map<BLOCK_ADDR, BLKGRAPH> GRAPH;

class aarch64Asm : public Code<options::aarch64::PASSES>
{
public:
  std::map<std::string, map<BLOCK_ADDR, Json::Value *>> functions;
public:
  ...
  void getFormattedCode(std::string &function, FUNCTION *code);
  void removeDataTransfer(std::string &function, map<INSADDR, INSTRUCTION> &code);
  void printRawCode(std::string &function);
  void PrettyPrintCode(FUNCTION &code);
  void PrettyPrintBlock(BLOCK &code);

  void DDAnalyze(string &function, FUNCTION &code, GRAPH &ddg);

  void PrettyPrintDDG(FUNCTION &code, GRAPH &ddg);
  void getColored(GRAPH &ddg);
  void PrettyPrintColor(GRAPH &ddg);
  void PrettyPrintGraph(vector<NODE> &g, map<INSADDR, INSTRUCTION> &code);
  void getColorField(BLKGRAPH &ddg);

private:
  void removeJmp(BLOCK &);
  void removeNop(FUNCTION *code);
  void extractRawCode(std::string &function, RAWFUNC *code, bool replace = true);
  ...
  bool isNop(std::string &mnemonic);
  void buildReturnValue(OPERAND &ret);
  void buildParam(FUNCTION *code);
  void fixReturnValue(FUNCTION *code);
  ...
  void buildDDG(FUNCTION &code, GRAPH &ddg);
  void findAllExits(GRAPH &ddg);
  
  void PrettyPrintExits(BLKGRAPH &ddg);
  ...
  void removeCsel(BLOCK &ins);
  // void removeVoid(map<INSADDR, INSTRUCTION> &code);
  void removeFunctionPreface(FUNCTION &code);
  void removeFunctionEpilogue(FUNCTION &code);
  void colorLeftNodes(BLKGRAPH &ddg, std::vector<INDEX> &nodes);
  void memoryLocalize(map<INSADDR, INSTRUCTION> &code);
  void removeADRP(FUNCTION &code);
...
// machine-related optimize
  ...
  static string get_op(string mnemonic, int idx) {
    std::stringstream ins_stream(mnemonic);
    std::string word;
    vector<string> ins_vec;
    while (ins_stream >> word) {
        ins_vec.push_back(word);
    }
    return ins_vec.at(idx);
  }
};
} //aarch64
} // namespace code
