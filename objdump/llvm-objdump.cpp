...
static void disassembleObject(const Target *TheTarget, const ObjectFile *Obj,
                              ...
                              PrettyPrinter &PIP,
                              SourcePrinter &SP, bool InlineRelocs, raw_string_ostream &OS) {
    ...
    for (unsigned SI = 0, SE = Symbols.size(); SI != SE; ++SI) {
      ...
      while (Index < End) {
        ...
        PIP.printInst(*IP, Disassembled ? &Inst : nullptr,
                      ...
                      OS, "", *STI, &SP, Obj->getFileName(), &Rels);
        OS << CommentStream.str();
        Comments.clear();
...
static void disassembleObject(const ObjectFile *Obj, raw_string_ostream &OS, bool InlineRelocs) {
...
static void dumpObject(ObjectFile *O, raw_string_ostream &OUTS, const Archive *A = nullptr,
  ...
  if (Disassemble){
    disassembleObject(O, OUTS, Relocations);
  }
  if (UnwindInfo)
    printUnwindInfo(O);
...
static void dumpObject(const COFFImportFile *I, const Archive *A, raw_string_ostream &OS,
                       const Archive::Child *C = nullptr) {
  StringRef ArchiveName = A ? A->getFileName() : "";
...
static void dumpArchive(const Archive *A, raw_string_ostream &OS) {
...
static std::string dumpInput(StringRef file) {
...
static std::string analyzeAsm(std::string &ASMCode, std::string filename) {
  std::string section_pat = "\\<sectionstart\\>\\nDisassembly of section (.*?):([\\s\\S]*?)\\<sectionend\\>";
  std::string segment_pat = "\\<segmentstart\\>\\n[0-9a-f]+ (.*):([\\s\\S]*?)\\<segmentend\\>";
  std::string inst_pat = "[\\s]+([0-9a-f]+):[\\s]+([^\\<\\>\\n]+)[\\s]{0,1}([\\<]+(.*)[\\>]+)*";
  std::string asm_pat = "[\\s]*\\.text[\\s]+.*?\\/\\/[\\s]*<MCInst\\s#([\\S]*)\\s([\\S]*)[\\s]+([\\s\\S]*)>[^<>]*";
  std::string inst_ori_pat = "([\\S]*)\\t(.*?)[\\s]*";
  std::string operand_pat = "<MCOperand (.*?):(.*)>";
  std::regex secreg(section_pat);
  std::regex segreg(segment_pat);
  std::regex instreg(inst_pat);
  std::regex asmreg(asm_pat);
  std::regex operandreg(operand_pat);
  std::regex inst_ori_reg(inst_ori_pat);
  std::smatch asmm, instm;
  // llvm::outs() << ASMCode << "\n";
  std::string jstr;
  llvm::raw_string_ostream JS(jstr);
  JS << "{\"filename\":";
  char abs_path[200];
  ...
  JS << "\"sections\":";
  JS << "[";
  std::sregex_iterator sec_pos(ASMCode.cbegin(), ASMCode.cend(), secreg);
  std::sregex_iterator sec_end;
  for (; sec_pos != sec_end; sec_pos++)
  {
    JS << "{\"section\":";
    JS << "\"" << sec_pos->str(1) << "\"";
    JS << ",";
    JS << "\"functions\":";
    JS << "[";
    std::string sec = sec_pos->str(2);
    std::sregex_iterator seg_pos(sec.cbegin(), sec.cend(), segreg);
    std::sregex_iterator seg_end;
    for (; seg_pos != seg_end; seg_pos++)
    {
      ...
      JS << "\"instructions\":";
      JS << "[";
      std::string insstr = seg_pos->str(2);
      // llvm::outs() << seg_pos->str(2);
      std::sregex_iterator ins_pos(insstr.cbegin(), insstr.cend(), instreg);
      std::sregex_iterator ins_end;
      for (; ins_pos != ins_end; ins_pos++){
        JS << "{\"address\":";
        JS << "\"" << ins_pos->str(1) << "\"";
        JS << ",";
        JS << "\"instruction\":";
        ...
        JS << ",";
        // Parse instruction
        std::string inst = ins_pos->str(2);
        std::string tmps;
        raw_string_ostream SS(tmps);
        buffer_ostream Out(SS);
        llvm::StringRef instRef(inst);
        int res = parserInstruction(MCOptions, instRef, Out);
        std::string parse = Out.str();
        bool mat = std::regex_match(parse, asmm, asmreg);
        // llvm::outs() << parse << "\n\n";
        JS << "\"opcode\":";
        JS << "\"" << asmm[1] << "\"";
        JS << ",";
        ...
        JS << "\"operands\":";
        JS << "[";
        std::string operands = asmm[3];
        std::sregex_iterator operand_pos(operands.cbegin(), operands.cend(), operandreg);
        std::sregex_iterator operand_end;
        for (; operand_pos != operand_end; operand_pos++)
        {
          JS << "{";
          JS << "\"type\":";
          JS << "\"" << operand_pos->str(1) << "\"";
          JS << ",";
          JS << "\"value\":";
          JS << "\"" << operand_pos->str(2) << "\"";
          JS << "}"; //operands·

          std::sregex_iterator tmp3 = operand_pos;
          if (++tmp3 != operand_end) JS << ",";
        }
        JS << "]"; //operands
        JS << "}"; //address
        std::sregex_iterator tmp2 = ins_pos;
        if (++tmp2 != ins_end) JS << ",";
      }
      JS << "]"; //instructions
      JS << "}"; // function
      std::sregex_iterator tmp1 = seg_pos;
      if (++tmp1 != seg_end) JS << ",";
    }
    JS << "]"; //functions
    JS << "}"; //section
    std::sregex_iterator tmp = sec_pos;
    if (++tmp != sec_end) JS << ",";
  }
  JS << "]"; //sections
  JS << "}"; //file
...
int dumpToJson(std::string &JS, std::string filename){
  Json::Reader reader;
  Json::Value root;
  if (reader.parse(JS, root, false))
  {
    std::ofstream os;
...
int disasm(std::string &filename, std::string &jsonfile) {
  using namespace llvm;
  // InitLLVM X(optnum, option);
  ...
  // cl::ParseCommandLineOptions(optnum, option, "llvm object file dumper\n");
  ...
  std::string ASMCode = dumpInput(filename);
  std::string JS = analyzeAsm(ASMCode, filename);
  int res = dumpToJson(JS, jsonfile);
  return EXIT_SUCCESS;
}

// int oneDis(std::string dir, int id, int opt){
//   std::string path = dir + "/case" + to_string(id) + "/o" + to_string(opt);
//   std::string objfile = path + "/main_o" + to_string(opt) + "-aarch64.o";
//   std::string outfile = path + "/main_o" + to_string(opt) + "-aarch64-dis.json";
//   std::string str;
